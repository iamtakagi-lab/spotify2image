...
export const Service: React.FC<{}> = () => {
  ...
  const submit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (url) {
      ...
      setImageUrl(
        url.replace(SPOTIFY_URL_REGEX, (_: any, item: string) => `https://spotify2image.vercel.app/image/${item}#.png`),
      );
    }
  };
  ...
  return (
    <>
      ...
      >
        ...
        <button
          type="submit"
          className="ml-2 transition duration-500 ease-in-out whitespace-nowrap block items-center justify-center border border-transparent 
                px-2 py-2 my-1 rounded-full shadow-sm text-base font-medium text-white bg-green-500 hover:bg-transparent hover:border-green-500 hover:text-green-500"
        >
          生成
        </button>
      </form>
